<x-app-layout title="New Transaction">
    <div class="mb-6">
        <h1 class="page-title">New Coin Transaction</h1>
        <p class="page-subtitle">Load coins into the machine or dispense to customers</p>
    </div>

    <div class="grid gap-6 lg:grid-cols-2">
        <div class="card">
            <h3 class="section-title mb-3">Current Stock</h3>
            <div class="grid grid-cols-2 gap-3">
                @foreach ($inventory as $item)
                    <div class="app-tile">
                        <p class="text-xs text-powder-dark">₱{{ $item->coin_value }}</p>
                        <p class="text-lg font-bold text-offwhite">{{ number_format($item->quantity) }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('user.transactions.store') }}" class="space-y-5" id="transaction-form">
                ...
                <div>
                    ...
                    <div class="mt-2 flex gap-4">
                        <label class="flex items-center gap-2 text-sm text-offwhite">
                            <input type="radio" name="type" value="load" class="form-radio" checked />
                            <span>Load (add coins)</span>
                        </label>
                        <label class="flex items-center gap-2 text-sm text-offwhite">
                            <input type="radio" name="type" value="dispense" class="form-radio" />
                ...
                <div class="rounded-lg border border-charcoal-border bg-charcoal-deep p-4">
                    <p class="text-sm text-powder">Estimated total</p>
                    <p class="text-3xl font-bold text-powder-light" id="total-display">₱0</p>
                </div>
